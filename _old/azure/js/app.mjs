    ...
    .then((deviceList) => {
      for (let deviceInfo of deviceList) {
        ...
        if (deviceInfo.kind === "videoinput") {
          ...
          if (
            deviceInfo.label &&
            deviceInfo.label.toLowerCase().includes(" ir ")
          )
            continue;
      ...
      selectedDevice = getCookie("selectedDevice")
        ? getCookie("selectedDevice")
        : 0;
    })
    .then(openCamera)
...
function openCamera() {
  ...
  navigator.mediaDevices
    ...
    .catch((err) =>
      showError(
        err.toString() +
          "<br>Make sure you accept camera permissions<br><a href='javascript:location.reload()'>Try reloading page</a>"
      )
    );
}
...
function captureImage() {
  ...
  offscreen
    .getContext("2d")
    .drawImage(video, 0, 0, vidDim.width, vidDim.height);
  ...
  else offscreen.toBlob(analyzePhotoFaceDetect, "image/jpeg");
}
